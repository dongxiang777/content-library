  ...
  const methods = {
    ...
    connect() {
      return new Promise((resolve, reject) => {
        const urls =
          typeof WXEnv.downloaderWSCandidates === "function"
            ? WXEnv.downloaderWSCandidates()
            : [WXEnv.downloaderWSURL];
        let idx = 0;
        let settled = false;
        ...
        function tryNext() {
          ...
          if (idx >= urls.length) {
            settled = true;
            reject(new Error("download ws: all candidates failed"));
            return;
          }
          const ws_url = urls[idx++];
          console.log("[DOWNLOADER]download ws connecting", ws_url);
          let ws;
          try {
          ...
          ws.onclose = () => {
            ...
            WXU.error({ msg: "download ws连接已关闭，请刷新页面", alert: 0 });
            if (WXU.downloader) {
