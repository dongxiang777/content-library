...
func CreateChannelInterceptorPlugins(interceptor *Interceptor, files *ChannelInjectedFiles) []*proxy.Plugin {
	...
	// Log search-related MITM traffic (搜一搜 / finder / mmsearch) for reverse-engineering.
	// 搜一搜主路径实际是 native getSearchData（非纯 HTTP），但若出现 HTTP 会打印在此。
	searchLogPlugin := &proxy.Plugin{
		Match: "*.weixin.qq.com",
		OnRequest: func(ctx proxy.Context) {
			...
			lower := strings.ToLower(host + full)
			if !(strings.Contains(lower, "search") ||
				strings.Contains(lower, "finder") ||
				strings.Contains(lower, "mmsearch") ||
				strings.Contains(host, "szextshort")) {
				return
			}
		...
		OnResponse: func(ctx proxy.Context) {
			...
			if strings.Contains(req.URL.Path, "get_feed_info") {
				body, err := ctx.GetResponseBody()
				if err == nil && len(body) > 0 {
					outPath := "data/feed_info_response.json"
					_ = os.WriteFile(outPath, body, 0644)
					fmt.Printf("[FEED-INFO] 已保存响应体到 %s (%d bytes)\n", outPath, len(body))
				}
			}
		},
	}
	...
	plugin1 := &proxy.Plugin{
		...
		OnRequest: func(ctx proxy.Context) {
			...
			if pathname == "/__wx_channels_api/tip" {
				...
				if data.Replace == 1 {
					// In-place progress update (no trailing newline).
					fmt.Printf("\r\033[K%v%s", *prefix, data.Msg)
				} else if data.End == 1 {
					// End a replace-style line; still print msg when present so
					// one-shot tips like "channels ws 已连接" are visible.
					if data.Msg != "" {
						fmt.Printf("\n%v%s\n", *prefix, data.Msg)
					} else {
						fmt.Println()
		...
		OnResponse: func(ctx proxy.Context) {
			...
			if hostname == "res.wx.qq.com" && strings.Contains(resp_content_type, "application/javascript") {
				...
				if strings.Contains(pathname, "virtual_svg-icons-register.publish") {
					...
					{
						// Keep method-style this binding: original body uses this.post(...).
						// Do not extract the method to a free function when calling from inject JS.
						js_finder_pc_search := `async finderPCSearch($1) {
					var result = await (async () => {
						$2;
					})();
					console.log("[DOWNLOADER]finderPCSearch", $1, result && result.data && {
						acct: (result.data.acctList||[]).length,
						obj: (result.data.objectList||[]).length,
						live: (result.data.liveObjectList||[]).length
					});
					return result;
				}async`
					...
					{
						// Log full request (incl. sortType/timeRange/extReqParams) so MITM
						// console shows whether 搜一搜-style filters are present on the wire.
						js_finder_search := `async finderSearch($1) {
					try {
						console.log("[DOWNLOADER]finderSearch req", JSON.stringify($1));
					} catch (e) {
						console.log("[DOWNLOADER]finderSearch req", $1);
					}
					var result = await (async () => {
						$2;
					})();
					console.log("[DOWNLOADER]finderSearch", $1, result && result.data && {
						info: (result.data.infoList||[]).length,
						obj: (result.data.objectList||[]).length,
						cont: result.data.objectContinueFlag,
	...
	// Keep plugin1 first: tests and callers expect channels host plugin at index 0.
	return []*proxy.Plugin{plugin1, plugin2, searchLogPlugin}
}
...
func CreateSimpleChannelInterceptorPlugin(interceptor *Interceptor, files *ChannelInjectedFiles) *proxy.Plugin {
	...
	v := "?t=" + version
	return &proxy.Plugin{
		// Only MITM channels host — never broad "qq.com" (breaks video CDN / init).
		Match: "channels.weixin.qq.com",
		OnRequest: func(ctx proxy.Context) {
