...
func NewChannelInjectedFiles(injectDir string) *ChannelInjectedFiles {
	...
	if rootFS := embeddedRootFS(); rootFS != nil {
		...
		// Fail loudly when running without -tags embed_inject and the inject
		// tree is missing. Otherwise mockChannelStaticAsset silently fails and
		// WeChat returns HTML for /__wx_channels_assets/* paths — the browser
		// then executes HTML as JS, so WebSocket never connects and the page
		// stays blank.
		if _, err := files.ReadSrc("channels.js"); err != nil {
			fmt.Printf(
				"警告: 无法读取 inject 资源 (dir=%s, err=%v)。\n"+
					"      请使用: go build -tags embed_inject ...\n"+
					"      或确保 inject 目录位于可执行文件旁/工作目录可访问。\n",
				injectDir, err,
			)
		}
	}
...
func findChannelInjectDir() string {
	...
	if exe, err := os.Executable(); err == nil {
		...
		candidates = append(candidates,
			...
			// Common layout: binary in tools/wx_channels_download/,
			// sources in tools/wx_channels_download_src/
			filepath.Join(exeDir, "..", "wx_channels_download_src", defaultChannelInjectDir),
			filepath.Join(exeDir, "..", filepath.Base(exeDir)+"_src", defaultChannelInjectDir),
		)
	}
	if wd, err := os.Getwd(); err == nil {
		candidates = append(candidates,
			filepath.Join(wd, defaultChannelInjectDir),
			filepath.Join(wd, "inject"),
			filepath.Join(wd, "..", "wx_channels_download_src", defaultChannelInjectDir),
		)
	}
