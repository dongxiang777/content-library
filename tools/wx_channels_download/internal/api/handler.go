...
func (c *APIClient) handleSearchChannelsVideo(ctx *gin.Context) {
	keyword := ctx.Query("keyword")
	// Default scene=19 (MMFinder_Search_Scene_PC_Main_Page) — required for video/feed results.
	scene_str := ctx.DefaultQuery("scene", "19")
	next_marker := ctx.Query("next_marker")
	scene, _ := strconv.Atoi(scene_str)
	offset, _ := strconv.Atoi(ctx.DefaultQuery("offset", "0"))

	body := types.ChannelsVideoSearchBody{
		Keyword:        keyword,
		Scene:          scene,
		NextMarker:     next_marker,
		Offset:         offset,
		RequestID:      ctx.Query("request_id"),
		FinderUsername: ctx.Query("finder_username"),
	}
	// sort: 0综合 1最新 2最热（对齐搜一搜结果页「排序」）
	if s := ctx.Query("sort"); s != "" {
		if v, err := strconv.Atoi(s); err == nil {
			body.Sort = &v
		}
	}
	// time_range: 0不限 1一天 2七天 3半年；亦接受天数 7/180
	if s := ctx.Query("time_range"); s != "" {
		if v, err := strconv.Atoi(s); err == nil {
			body.TimeRange = &v
		}
	}
	// scope: 0不限 1已关注 2最近看过 3朋友赞过（PC finderSearch 常无效，仍透传）
	...
	resp, err := c.channels.SearchChannelsVideo(body)
	if err != nil {
		result.Err(ctx, 400, err.Error())
		return
	}
