...
// FlexInt unmarshals JSON numbers or numeric strings (WeChat APIs often
// serialize large integers such as fileSize as strings for JS safety).
type FlexInt int
...
func (f *FlexInt) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &f64); err != nil {
		return fmt.Errorf("FlexInt: cannot unmarshal %s: %w", string(data), err)
	}
	*f = FlexInt(int(f64))
	return nil
}
...
type ChannelsMediaItem struct {
	...
	VideoPlayLen FlexInt             `json:"videoPlayLen"`
	Width        float32             `json:"width"`
	Height       float32             `json:"height"`
...
type ChannelsObjectDesc struct {
	...
	FinderNewlifeDesc json.RawMessage        `json:"finderNewlifeDesc,omitempty"`
}
...
// ChannelsVideoSearchBody is sent to the WeChat channels page via websocket.
// Scene values used by PC frontend (search.publish):
//
//	13 = Live, 19 = PC_Main_Page (accounts + feeds), 20 = Within_UserPage,
//	21 = PC_Account, 22 = PC_MegaVideo, 23 = PC_Live.
//
// Video/feed results require scene=19 (default).
//
// Filters align with 搜一搜 result-page dropdowns (and TikHub publish_time/sort):
//
//	sort:       0=综合(default) 1=最新 2=最热
//	time_range: 0=不限, 1=一天, 2=七天, 3=半年
//	            also accepts day counts 7 / 180 (mapped to enum 2 / 3)
//	scope:      0=不限 1=已关注 2=最近看过 3=朋友赞过
//
// Note: PC channels finderSearch often ignores server-side filters; the inject
// layer also applies local time/sort on objectList so results match the request.
type ChannelsVideoSearchBody struct {
	Keyword        string `json:"keyword"`
	Scene          int    `json:"scene"`
	NextMarker     string `json:"next_marker"`
	Offset         int    `json:"offset,omitempty"`
	RequestID      string `json:"request_id,omitempty"`
	Sort           *int   `json:"sort,omitempty"`            // 0综合 1最新 2最热
	TimeRange      *int   `json:"time_range,omitempty"`      // 0不限 1一天 2七天 3半年（亦接受 7/180 天数）
	Scope          *int   `json:"scope,omitempty"`           // 0不限 1已关注 2最近看过 3朋友赞过
	FinderUsername string `json:"finder_username,omitempty"` // scene=20 号内搜索
}

// ChannelsVideoSearchResp mirrors finderSearch response for video/feed search.
// objectList holds video feeds; infoList may still contain accounts when scene=19.
type ChannelsVideoSearchResp struct {
	ErrCode int    `json:"errCode"`
	ErrMsg  string `json:"errMsg"`
	Data    struct {
		BaseResponse       BaseResponse     `json:"BaseResponse"`
		InfoList           []InfoListItem   `json:"infoList"`
		ContinueFlag       int              `json:"continueFlag"`
		ObjectContinueFlag int              `json:"objectContinueFlag"`
		LastBuff           string           `json:"lastBuff"`
		...
		MultiFeedStream    []any            `json:"multiFeedStream"`
		ObjectList         []ChannelsObject `json:"objectList"`
		LiveObjectList     []ChannelsObject `json:"liveObjectList"`
		// LocalFilter is set by inject when client-side sort/time filtering ran.
		LocalFilter json.RawMessage `json:"_localFilter,omitempty"`
	} `json:"data"`
	Payload struct {
		Query     string `json:"query"`
		Scene     int    `json:"scene"`
...
func ChannelsObjectToChannelsFeedProfile(r *ChannelsObject) *ChannelsFeedProfile {
	...
	prof := &ChannelsFeedProfile{
		...
		Duration:    media.VideoPlayLen.Int(),
		FileSize:    media.FileSize.Int(),
		CreatedAt:   feed.CreateTime.Int(),
