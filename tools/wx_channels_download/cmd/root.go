...
func root_command(cfg *config.Config) {
	...
	if !buildtags.UsingSunnyNet {
		if interceptor_cfg.ProxyTun {
			...
			proxy_settings := interceptor_srv.Interceptor.SystemProxySettings()
			color.Green("已设置选择性 PAC 系统代理（仅微信/腾讯相关域名走 MITM）")
			color.Green(fmt.Sprintf("PAC: %s", proxy_settings.PACURL))
			if proxy_settings.Fallback != "" {
				color.Green(fmt.Sprintf("其他流量回落: %s", proxy_settings.Fallback))
			} else {
				color.Green("其他流量: DIRECT（不影响 Chrome 等正常上网）")
			}
			color.Green("请打开需要下载的视频号页面进行下载")
			expected_key := system.AddrKey(proxy_settings)
			// Clash/Vortex 等会周期性覆盖系统代理。检测到偏离时自动恢复 PAC，
			// 而不是把全局 HTTP 代理抢回 2023（那会劫持 Chrome）。
			go func() {
				ticker := time.NewTicker(5 * time.Second)
				defer ticker.Stop()
				var last_warn time.Time
				for {
					select {
					case <-ctx.Done():
						return
					case <-ticker.C:
						cur, err := system.FetchCurProxy(system.ProxySettings{Device: proxy_settings.Device})
						if err != nil {
							continue
						}
						cur_key := system.CurAddrKey(cur)
						if cur_key == expected_key {
							continue
						}
						if err := system.EnableProxy(proxy_settings); err != nil {
							if time.Since(last_warn) > 30*time.Second {
								color.Red("\n系统代理已被修改且自动恢复失败: %v (期望 %s, 当前 %s)", err, expected_key, cur_key)
								last_warn = time.Now()
							}
							continue
						}
						if time.Since(last_warn) > 30*time.Second {
							color.Yellow("\n系统代理已被修改（当前 %s），已自动恢复为选择性 PAC %s", cur_key, expected_key)
							last_warn = time.Now()
						}
					}
				}
