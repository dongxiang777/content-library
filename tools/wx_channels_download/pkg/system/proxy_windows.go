...
func enable_proxy(args ProxySettings) error {
	...
	// Clear AutoConfigURL so PAC does not compete with manual proxy.
	_ = exec.Command("reg", "add", path, "/v", "AutoConfigURL", "/t", "REG_SZ", "/d", "", "/f").Run()
...
func enable_pac_proxy(args ProxySettings) error {
	...
	// Disable manual proxy so only PAC applies — otherwise WinINET may prefer
	// ProxyServer and hijack all browser traffic.
	cmd := exec.Command("reg", "add", path, "/v", "ProxyEnable", "/t", "REG_DWORD", "/d", "0", "/f")
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("关闭手动系统代理失败，%v", string(output))
	}
	cmd = exec.Command("reg", "add", path, "/v", "AutoConfigURL", "/t", "REG_SZ", "/d", pacURL, "/f")
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("设置 PAC 失败，%v", string(output))
	}
	notify_proxy_changed()
	return nil
}
...
func fetch_cur_proxy(args ProxySettings) (*ProxySettings, error) {
	...
	// Prefer PAC when AutoConfigURL is set.
	pacURL, err := read_reg_value(path, "AutoConfigURL")
	if err != nil {
		return nil, err
	}
	pacURL = strings.TrimSpace(pacURL)
	if pacURL != "" {
		return &ProxySettings{PACURL: pacURL}, nil
	}
...
func notify_proxy_changed() {
	// Best-effort: InternetSetOption via powershell is heavy; WinINET often
	// picks up registry changes for new connections. Leave as no-op for Win7
	// compatibility (existing code path never notified either).
}
