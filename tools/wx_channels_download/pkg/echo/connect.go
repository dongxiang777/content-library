...
func (h *ConnectHandler) getMitmServer(hostname string) (*MitmServer, error) {
	...
	// Bind certificate generation to the CONNECT hostname so clients that
	// omit SNI (or send an unexpected name) still get a matching leaf cert.
	// Present a full chain (leaf + CA) for Chromium/WebView trust path building.
	// Force HTTP/1.1 on the MITM face: WeChatAppEx/Chromium can be picky about
	// HTTP/2 over synthetic certs, and body rewriting is simpler on h1.
	server := &http.Server{
		...
		TLSConfig: &tls.Config{
			...
			GetCertificate: func(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
				...
				return h.CertManager.GetCertificate(name)
			},
		},
		// Empty non-nil map disables HTTP/2 on this server.
