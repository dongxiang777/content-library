#!/usr/bin/env python3
"""
白大姐账号矩阵采集：所有昵称含"白大姐"的账号，点赞>=1000，下载+转写+写入情感表。
"""
import sys
import time
...
print(f"=== 白大姐矩阵采集 (点赞>={MIN_LIKES}, 业务={BUSINESS}) ===", flush=True)

# 1. 搜索账号，过滤昵称含"白大姐"
accounts = search_accounts(KEYWORD, max_pages=2)
matched = [a for a in accounts if KEYWORD in a.get("nickname", "")]
print(f"匹配账号: {len(matched)} 个", flush=True)
for a in matched:
...
for i, acc in enumerate(matched):
    ...
    try:
        items = collect_from_username(
            uname,
            count=200,
            fetch_engagement=False,  # 列表已自带，不需要逐条请求
            min_likes=MIN_LIKES,
        )
        print(f"  → {len(items)} 条过千视频", flush=True)
...
# 保存原始数据
raw_path = Path(DATA_DIR) / f"channels_baidajie_{date.today()}.json"
_save_raw_items(new_items, raw_path)

# 4. 并行 pipeline（下载+转写+分类+写飞书）
